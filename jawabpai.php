...
<body>
    ...
    <?php
        ...
        $query = "SELECT * FROM ujian WHERE id_user = {$_SESSION['user']['id_user']} and id_matkul = 3";
        $result = $db->query($query);

        if ($result->num_rows) {
            $sql = "UPDATE ujian SET nilai = '$score' WHERE id_user = '{$_SESSION['user']['id_user']}' and id_matkul = 3";
        } else {
            $sql = "INSERT INTO `ujian`(`id_ujian`, `id_user`, `id_matkul`, `nilai`, `status`) VALUES (null,'{$_SESSION['user']['id_user']}', '3','{$score}','Belum Terkirim')";
        }
