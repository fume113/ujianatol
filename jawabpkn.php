...
<body>
    ...
    <?php
        ...
        $query = "SELECT * FROM ujian WHERE id_user = {$_SESSION['user']['id_user']} and id_matkul = 2";
        $result = $db->query($query);

        if ($result->num_rows) {
            $sql = "UPDATE ujian SET nilai = '$score' WHERE id_user = '{$_SESSION['user']['id_user']}' and id_matkul = 2";
        } else {
            $sql = "INSERT INTO `ujian`(`id_ujian`, `id_user`, `id_matkul`, `nilai`, `status`) VALUES (null,'{$_SESSION['user']['id_user']}', '2','{$score}','Belum Terkirim')";
        }
