...
<head>
    ...
    <title>Soal Pilihan Ganda Mata Kuliah Pendidikan Agama Islam</title>
    <style>
        * {
...
<body>
    ...
    <table border="0">
        <tbody>
            <?php
            include "koneksi.php";
            $query    = mysqli_query($db, "SELECT * FROM soal WHERE id_matkul = '2' AND aktif='Y' ORDER BY id_soal DESC");
            $jumlah = mysqli_num_rows($query);
            $no = 0;
            while ($data = mysqli_fetch_array($query)) {
                $no++
            ?>
                <form action="jawabpkn.php" method="POST">
